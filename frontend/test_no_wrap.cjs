...
function parseChordProBlocks(chordproText, paginationOptions = null) {
  ...
  for (const block of rawBlocks) {
    if (paginationOptions && paginationOptions.availablePx) {
      const { availablePx, lyricHeightPx, chordHeightPx, showChords } = paginationOptions; // NO availableWidthPx
      
      const lineHeights = block.lines.map(line => {
        return lyricHeightPx; // NO WRAPPING
      });
...
for (let m = 3.5; m >= 1.0; m -= 0.1) {
  ...
  if (paginatedBlocks.length < minSplits) {
    minSplits = paginatedBlocks.length;
    bestMultiplier = m;
  }
}
console.log("bestMultiplier NO WRAP:", Math.round(bestMultiplier * 10) / 10);
