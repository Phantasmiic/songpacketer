...
(async () => {
  ...
  if (await fullSongBtn.isVisible()) {
    // Check if it's currently selected. If it has variant="outlined", it's off. 
    // Material UI buttons have classes. 
    // Let's just click it to ensure it triggers the layout.
    await fullSongBtn.click();
    await page.waitForTimeout(1000); 
    await page.screenshot({ path: '/Users/david/.gemini/antigravity-ide/brain/c1b854b1-f6fe-4c06-8f88-d423c10df7d6/song1_full.png' });
  ...
  await page.waitForTimeout(2000);
  if (await fullSongBtn.isVisible()) {
    // Ensure we are still in full song mode
    await page.screenshot({ path: '/Users/david/.gemini/antigravity-ide/brain/c1b854b1-f6fe-4c06-8f88-d423c10df7d6/song18_full.png' });
